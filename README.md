<h1 align="center">你好，我是黄栩 👋</h1>

<h3 align="center">📊 数据分析 / 数据科学方向学习中 🔎</h3>

<p align="center">
  ✨ 欢迎来到我的 GitHub 主页 ✨ <br><br>
  我目前在日本东北大学学习，主要关注数据分析、因果推断、SQL 和机器学习。<br>
  我希望把复杂的数据整理成清晰的分析，把技术方法真正用于理解业务问题。<br>
  也希望在这里持续记录自己的学习过程和成长轨迹。
</p>

---

## 🛠 技术栈

我目前常用 / 正在接触的工具和技术：

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-336791?style=flat-square&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white" />
  <img src="https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white" />
  <img src="https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white" />
  <img src="https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white" />
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black" />
  <img src="https://img.shields.io/badge/ECharts-AA344D?style=flat-square" />
  <img src="https://img.shields.io/badge/Git-222222?style=flat-square&logo=git&logoColor=white" />
</p>

---

## 🌱 我目前正在学习的内容

我最近主要在提升这些方向：

- SQL
- Python 数据清洗与分析
- 面向业务问题的数据分析思路
- 因果推断与机器学习
- 数据可视化与分析表达

---

## 🤝 联系我

欢迎交流：

- Email：`huangxucsdm@gmail.com`

---

## ✨ 很高兴你来到这里

希望我能在这里持续积累、持续输出，  
一步一步把数据分析和数据科学真正学扎实。

如果你也对数据分析、因果推断或商业分析感兴趣，欢迎交流。
